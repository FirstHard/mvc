<?php
	...
	/*class DbLink {
	    private static $db_creds = [
	        ...
	        'host' => 'localhost',
	        'password' => '14041977',
	        'db_name'    => 'hotlingerie_test'
	    ];

		static function dbase() {
			...
			if (is_null($mysqli)) {
				$mysqli = new mysqli(
					self::$db_creds['host'],
					self::$db_creds['user'],
					self::$db_creds['password'],
					self::$db_creds['db_name']
				);

				if ($mysqli->connect_errno) {
					// в следующий вызов попробуем еще раз
					$mysqli = $mysqli->connect_errno;
				}
			}
			return $mysqli;
		}

		static function get_data($action, $selection) {
			$action = ''; // SELECT, COUNT, etc.
